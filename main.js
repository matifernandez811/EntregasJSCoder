...
function mensajePrompt(texto){
    return prompt(texto)
}
let nombreUsuario = mensajePrompt(`Ingrese su nombre`)
    mensajeAlert(`Bienvenido a nuestro ecommerce ${nombreUsuario}`)  

let interesCategoria = prompt("Que desea comprar? Elija la opcion 1 para remeras, opcion 2 pantalones,opcion 3 zapatos.")

let direccion
if(interesCategoria == 1){
let compraRemeras = confirm("Usted eligió remera. Su precio es de $500. Desea comprar este artículo?")
 if(compraRemeras === true){
     for(let i=1; i<=3; i++){
    if(i==1){
        direccion = mensajePrompt("Ingrese su dirección para poder realizar el envío del pedido") 
     }else if(i==2){
         mensajeAlert(`Su artículo será enviado a: ${direccion}`)
     }else if(i==3){
         mensajeAlert(`Muchas gracias por su compra ${nombreUsuario}`)
     }
 }
}else{
    mensajeAlert(`Muchas gracias por su visita ${nombreUsuario}`)
}

}
else if(interesCategoria == 2){
let compraPantalones = confirm("Usted eligió pantalones. Su precio es de $750. Desea comprar este artículo?")
if(compraPantalones === true){
    for(let i=1; i<=3; i++){
   if(i==1){
       direccion = mensajePrompt("Ingrese su dirección para poder realizar el envío del pedido") 
    }else if(i==2){
...
else if(interesCategoria == 3){
    let compraZapatos = confirm("Usted eligió zapatos. Su precio es de $1200. Desea comprar este artículo?")
    if(compraZapatos === true){
        for(let i=1; i<=3; i++){
       if(i==1){
           direccion = mensajePrompt("Ingrese su dirección para poder realizar el envío del pedido") 
        }else if(i==2){
...
}else if(interesCategoria !== 1,2,3){
    mensajeAlert(`Numero ingresado invalido, recorda ingresar 1,2 o 3`)
}
